...
//uint32_t nextSendTime = millis() + 60 * 60 * 1000;
...
void printResponseTime(uint32_t response, uint32_t  syncTime)
{

  Serial.print("\nNTP response time was : ");
  Serial.print(500 - response);
  Serial.println(" ms");

  Serial.print("Time since last sync is: ");
  Serial.print(syncTime / 1000.0);
  Serial.println(" s");
}
...
bool decodeNTP(void)
{
  ...
  while (millis() < waitTime && !timeValid)
  {
    ...
    if (udp.parsePacket())
    {
      ...
      printResponseTime((waitTime - newRecvTime), (newRecvTime - lastRecvTime) );
